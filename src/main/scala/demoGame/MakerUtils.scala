...
object MakerUtils {
  def makeCharacterControl(g: Spatial, speed:Float = 10f)(implicit app: SimpleApplication): CreatureMovementControl = {
    val cc = new CreatureMovementControl(.5f, 1.5f, 10f, speed)
    g.addControl(cc)
  ...
  var shadedMatsCache: mutable.Map[ColorRGBA, Material] = new mutable.HashMap()
  def makeShadedCached(color: ColorRGBA)(implicit app: SimpleApplication): Material = {
    shadedMatsCache.getOrElseUpdate(color, makeShaded(color))
  }
